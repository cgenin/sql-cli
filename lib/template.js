...
class TemplateManager {
    constructor() {
        ...
        const sqlCliHomeTemplatesDirectory = path.join(paths, '.sql-cli');
        this.templatesFolder = (process.env.NODE_ENV === 'developpement') ? './test/templates' : sqlCliHomeTemplatesDirectory;
    }

    getTemplates() {
        const b = fs.existsSync(this.templatesFolder);
        if (!b) {
            return throwError(new Error('Repository not initialised. Please call command "update"'));
        }
